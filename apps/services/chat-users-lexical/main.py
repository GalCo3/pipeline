from models import ChatUserMessage
from settings import get_settings

from hermes.connections import (
    BaseConsumerHandler,
    BaseElasticHandler,
    BaseMongoHandler,
)
from hermes.observability import (
    MessageStatus,
    TelemetryCounter,
    TelemetryHistogram,
    get_logger,
    init_observability,
    kafka_context,
)
from hermes.utils import send_to_dls, site_error, with_indexed_at

init_observability(service_name="chat-users-lexical")
logger = get_logger(__name__)
messages_processed = TelemetryCounter(
    "chat_users_lexical_messages_processed_total", allowed_labels=["status"]
)
messages_sent_to_dls = TelemetryCounter("chat_users_lexical_messages_dls_total")
message_duration = TelemetryHistogram(
    "chat_users_lexical_message_duration", unit="s", allowed_labels=["status"]
)


def main():
    settings = get_settings()
    consumer_handler = BaseConsumerHandler(settings.consumer_config)
    elastic_handler = BaseElasticHandler(settings.elastic_config)
    dls_handler = BaseMongoHandler(settings.mongo_config)

    for message in consumer_handler.start_consuming():
        try:
            with kafka_context(message, name="process_chat_user_message"):
                # model_validate, not **message.value(): the consumer
                # deserializes to a dict, but confluent types value() as bytes.
                chat_user = ChatUserMessage.model_validate(message.value())
                logger.info("Processing chat user message", doc_id=chat_user.id)

                # The source emits the full user record on every change, so an
                # id-keyed index is both the create and the update path.
                with message_duration.time(labels={"status": MessageStatus.INDEXED}):
                    local_response, remote_response = elastic_handler.index(
                        settings.index_name,
                        chat_user.id,
                        with_indexed_at(chat_user.model_dump(mode="json")),
                        is_multisite=True,
                    )
                    site_error(
                        local_response,
                        remote_response,
                        f"Failed to index chat-users-lexical document {chat_user.id}",
                    )
                logger.info("Successfully indexed chat user document", doc_id=chat_user.id)
                messages_processed.inc(labels={"status": MessageStatus.INDEXED})
        except Exception as e:
            logger.error(
                "Failed to process chat user message, sending to DLS",
                exc_info=True,
            )
            messages_processed.inc(labels={"status": MessageStatus.ERROR})
            messages_sent_to_dls.inc()
            send_to_dls(
                dls_handler, message, e, settings.mongo_config.database, settings.dls_collection
            )


if __name__ == "__main__":
    main()
