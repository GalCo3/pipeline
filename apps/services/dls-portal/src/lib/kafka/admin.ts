 ...
 * subscribe, no poll — a portal that joined a consumer group would move the
...
declare global {
  var __dlsPortalAdmin: Promise<KafkaJS.Admin> | undefined;
}

function admin(): Promise<KafkaJS.Admin> {
  if (!globalThis.__dlsPortalAdmin) {
    globalThis.__dlsPortalAdmin = (async () => {
      const instance = kafka().admin();
      await instance.connect();
      return instance;
    })().catch((error) => {
      globalThis.__dlsPortalAdmin = undefined;
      throw error;
    });
  }
  return globalThis.__dlsPortalAdmin;
}
