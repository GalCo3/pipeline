...
```
service consumer fails ──▶ hermes.dls (MongoDB) ──▶ dls-portal ──▶ browser
                                                        │
...
- **Mongo is the read path, Kafka the write path.** The portal browses, groups
...
`audit` — append-only, portal-owned: `messageId`, `action`
(`REPLAY | EDIT_REPLAY | DISCARD | CLEAR_HISTORY`), `actor`, `at`, `bulkId`,
`detail`, `result`, `error`. `bulks` — portal-owned bulk progress, in Mongo so it
survives a restart.
...
```bash
docker build -f apps/services/dls-portal/Dockerfile -t dls-portal:local .   # from the repo root
...
tools/scripts/sandbox/port-forward.sh                                               # portal on http://localhost:8086
```
...
Keycloak: realm `dls-portal`, client `dls-portal` with secret
`dev-portal-secret`, user `dev` / `dev`.
