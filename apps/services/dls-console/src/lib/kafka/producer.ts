...
/**
 * Replay producer — the console's ONLY Kafka write path.
 *
...
declare global {
  var __dlsConsoleProducer: Promise<KafkaJS.Producer> | undefined;
}
...
function producer(): Promise<KafkaJS.Producer> {
  if (!globalThis.__dlsConsoleProducer) {
    globalThis.__dlsConsoleProducer = (async () => {
      ...
      globalThis.__dlsConsoleProducer = undefined;
      throw error;
    });
  }
  return globalThis.__dlsConsoleProducer;
}
