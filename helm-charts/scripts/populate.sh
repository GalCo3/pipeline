...
# Infrastructure must already be installed (scripts/install.sh). This reuses the
# already-built demo-producer image: after editing produce.py or its example
# fixtures, re-run scripts/install.sh so a fresh image tag is built.
set -euo pipefail
...
echo "Done. Ten example messages per source produced (see demo-producer/examples);"
