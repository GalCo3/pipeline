...
# Infrastructure stays up; run scripts/populate.sh to recreate the indices and
...
CONSUMERS=(cargo-lexical chat-lexical chat-rooms-lexical chat-users-lexical chief-lexical)
...
# the es-index and demo-producer job pods to schedule into. scripts/populate.sh
# recreates the indices, produces the examples, and starts them again.
echo
echo "Done. Consumers are stopped — run scripts/populate.sh to recreate the"
