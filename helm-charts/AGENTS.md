...
- `local-infra/` — everything the services run against, in three buckets:
  - `backing/` — what a service addresses by DNS name, each store with its UI
    ...
    issuer `dls-console` logs in against), and `triton/` — a mock of the
    production Triton Inference Server the semantic path embeds against, run
    from the `tools/mock-triton` image.
